...
test.describe("Date picker tests @regression @ui", () => {
  test("select 10 days from today", async ({ page }) => {
    const pm = new PageManager(page);
    await pm.navigateTo().datepickerPage();
    await pm.onDatePickerPage().daysFromToday(10);
  });

  test("select 30 days from today", async ({ page }) => {
    const pm = new PageManager(page);
    await pm.navigateTo().datepickerPage();
    await pm.onDatePickerPage().daysFromToday(30);
  });

  test("select range 2 days to 5 days from today", async ({ page })=> {
    const pm = new PageManager(page);
    await pm.navigateTo().datepickerPage();
    await pm.onDatePickerPage().rangeFromToday(2, 5);
  });

  test("select range 5 days to 62 days from today", async ({ page })=> {
    const pm = new PageManager(page);
    await pm.navigateTo().datepickerPage();
